...
(function (global, factory) {
    ...
    class WordDocument {
        ...
        async loadDocumentChart(id, part) {
    ...
    class DocumentParser {
        ...
        parseChart(elem) {
            var result = { type: DomType.Chart, src: globalXmlParser.attr(elem, "id") || "", cssStyle: {} };
            return result;
        }
        parsePicture(elem) {
        ...
        parseIndentation(node, style) {
            ...
            var w = globalXmlParser.lengthAttr(node, "w");
            ...
            if (left || start || w)
                style["margin-inline-start"] = left || start || w;
            if (right || end)
    ...
    class ChartToSvgConverter {
        ...
        extractSeriesName(serElement, ns) {
        ...
        extractSeriesData(serElement, ns) {
        ...
        extractCategories(chartElement, ns) {
        ...
        extractNumericData(cacheElement, ns) {
            if (!cacheElement)
                return [];
        ...
        extractYAxisConfig(valAxElement, ns) {
            if (!valAxElement)
                return undefined;
        ...
        extractText(element, ns) {
            const richElement = element.getElementsByTagNameNS(ns.c, 'rich')[0];
            if (richElement) {
                richElement.getElementsByTagNameNS(ns.a, 'bodyPr')[0];
        ...
        extractSeriesColor(serElement, ns, index) {
        ...
        generateSvg(chartData, config) {
            ...
            const chartAreaHeight = height - padding * 2 - 50;
        ...
        drawTitle(svg, title, width, padding) {
        ...
        drawAxes(svg, chartData, chartAreaX, chartAreaY, chartAreaWidth, chartAreaHeight, padding) {
        ...
        drawLineChart(svg, chartData, chartAreaX, chartAreaY, chartAreaWidth, chartAreaHeight) {
        ...
        drawBarChart(svg, chartData, chartAreaX, chartAreaY, chartAreaWidth, chartAreaHeight) {
            const { series, categories, yAxis } = chartData;
            if (categories.length === 0 || series.length === 0)
                return;
            const allData = series.flatMap(series => series.data);
            ...
            const barGroupWidth = step * 0.7;
            const barWidth = barGroupWidth / seriesCount;
            const barSpacing = (step - barGroupWidth) / 2;
        ...
        drawPieChart(svg, chartData, chartAreaX, chartAreaY, chartAreaWidth, chartAreaHeight) {
        ...
        generatePieSlicePath(centerX, centerY, radius, startAngle, endAngle) {
        ...
        drawAreaChart(svg, chartData, chartAreaX, chartAreaY, chartAreaWidth, chartAreaHeight) {
            const { series, categories, yAxis } = chartData;
            if (categories.length === 0 || series.length === 0)
                return;
            const allData = series.flatMap(series => series.data);
